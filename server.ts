...
const seasonalFallbacks: Record<string, any[]> = {
  "봄": [
...
function getFallbackData(query: string): any[] {
...
async function startServer() {
  ...
  app.post("/api/recommendations", async (req, res) => {
    ...
    let context = typeof query === "string" ? query.trim() : "";
...
`;
    ...
    // Try multiple active models in priority order for resilience
    const modelsToTry = [
      ...
      try {
        ...
        if (text) {
          ...
          if (Array.isArray(parsed) && parsed.length > 0) {
            ...
            console.log(`Successfully generated flower recommendations with ${modelName}`);
            break;
          }
        }
      } catch (err: any) {
    ...
    if (generatedData && Array.isArray(generatedData)) {
      generatedData.sort((a: any, b: any) => (a.rank || 0) - (b.rank || 0));
      return res.json(generatedData);
    }

    // If all models encountered rate limits or connection errors, use curated fallback
    console.warn("Using curated gardening fallback data due to API error:", lastError?.message);
